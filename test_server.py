...
response = get(base_url+"serverKey", payload)
...
response = get(base_url+"encrypt", payload, cookies=cookies).json()
server_cipher = response["cipherText"]

payload = {"cipherText": "%X" % (client_cipher)}
response = get(base_url+"decrypt", payload, cookies=cookies).json()
...
payload = {"message": "%X" % (client_message)}
response = get(base_url+"sign", payload, cookies=cookies).json()
...
response = get(base_url+"verify", payload, cookies=cookies).json()
...
payload = {"modulus": "%X" % (client_n),
           "publicExponent": "%X" % (client_e)}
response = get(base_url+"sendKey", payload, cookies=cookies).json()
...
payload = {"key": "%X" % (client_new_key_crypted),
           ...
           "modulus": "%X" % (client_n),
           "publicExponent": "%X" % (client_e)}
response = get(base_url+"receiveKey", payload, cookies=cookies).json()
